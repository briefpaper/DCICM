# coding=utf-8
import os
from cqabaseline import evaluate_mean_ndcg_and_precision
from dcicm.config import num_neg_samples, knrm_train_data_path, knrm_test_data_path, num_ndcg_samples, \
    num_real_samples
...
from adhoc.knrm.model.model_base import BaseNN



# from ranking_measures.measures import find_ndcg
...
class Knrm(BaseNN):
    ...
    learning_rate = Float(0.01, help="learning rate, default is 0.001").tag(config=True)
    ...
    def model(self, inputs_q, inputs_d, mask, q_weights, mu, sigma):
        ...
        tmp = tf.exp(-tf.square(rs_sim - mu) / (tf.multiply(tf.square(sigma), 2)))
        ...
        feats_tmp = tf.concat(feats, axis=1)
    ...
    def train(self, train_pair_file_path, val_pair_file_path, train_size, checkpoint_dir, load_model=False):
        ...
        losses = tf.maximum(0.0, 1 - o_pos + o_neg)

        correct = tf.cast(tf.greater(o_pos, o_neg), tf.float32)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(int(self.max_epochs)):
                ...
                for BATCH in self.data_generator.pairwise_reader(pair_stream, self.batch_size, with_idf=True):
                    ...
                    # print("{}\t{}".format(step, self.eval_frequency))
                    n_val_batch = 0
                    if (step + 0) % self.eval_frequency == 0:
                        # print("============")
                        val_l = 0
                        ...
                        for BATCH in self.val_data_generator.pairwise_reader(val_pair_stream, self.batch_size, with_idf=True):
                            ...
                            # l, ls = sess.run([loss, losses], feed_dict=val_feed_dict)

                            # l, ls, correct_val, pos_val, neg_val = sess.run([loss, losses, correct, o_pos, o_neg], feed_dict=val_feed_dict)
                            # p_list.extend(list(pos_val.flatten()))
                            # n_list.extend(list(neg_val.flatten()))
                            pos_val = sess.run(o_pos, feed_dict=val_feed_dict)
                            # print(pos_val)
                            test_match_scores.extend(pos_val.flatten())

                            # print(correct_val.shape)

                            val_l += l
                            n_val_batch += 1

                            # correct_list.extend(correct_val.flatten())
                            #
                            # correct_count += (ls < 1).sum()
                        ...
                        # print("ave_ndcg = {}".format(ave_ndcg_score))
                        ...
                        start_time = time.time()
                        # print("epoch = {}")
                        print('Epoch = %d Step %d (epoch %.2f), %.1f ms per step' % (epoch, step,
...
if __name__ == '__main__':
    ...
    cmd_line = "sample.config --train --train_file {} --validation_file {} --train_size 20 --checkpoint_dir /home/hujun/data/MM2017/log".format(knrm_train_data_path, knrm_test_data_path)
    # cmd_line = "sample.config --train --train_file {} --validation_file {} --train_size 20 " \
    #            "--checkpoint_dir /data/AAAI2018/log".format(
    #     knrm_train_data_path, knrm_test_data_path)
