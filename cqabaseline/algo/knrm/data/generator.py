...
class DataGenerator(Configurable):
    ...
    def pointwise_generate(self, pair_stream, batch_size, with_label=True, with_idf=False):
        ...
        for line in pair_stream:
            cols = line.split('\t')
            q = np.array([int(t) for t in
                          cols[0].split(',')])  # np.loadtxt(StringIO(unicode(cols[0])), delimiter=',',  dtype=int,)
            doc = np.array([int(t) for t in
                            cols[1].split(',')])  # np.loadtxt(StringIO(unicode(cols[1])), delimiter=',',  dtype=int,)
...
if __name__ == '__main__':
    from adhoc.knrm.utils.base_conf import set_basic_log, load_py_config
